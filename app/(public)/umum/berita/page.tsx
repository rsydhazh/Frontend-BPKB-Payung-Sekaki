  ...
  useEffect(() => {
    async function fetchBerita() {
      try {
        ...
        // Filter data: Hanya ambil berita yang modul/kategorinya 'umum'
        const beritaUmum = data.filter((item: any) => 
          item.modul?.toLowerCase() === "umum" || item.category?.toLowerCase() === "umum"
        );
        setBeritaData(beritaUmum);
      } catch (error) {
        console.error("Gagal ambil berita umum:", error);
      } finally {
        setIsLoading(false);
      }
    }
    ...
    <main className="bg-[#fcfdff] min-h-screen pb-24 font-sans pt-14 relative">
      ...
      <section className="pt-2 pb-20 px-6 text-center max-w-3xl mx-auto relative">
        ...
        <p className="text-gray-500 text-lg max-w-2xl mx-auto font-medium leading-relaxed">
          Kumpulan artikel, pengumuman, dan berita terbaru seputar program Umum.
        </p>
      </section>

      {/* 2. AREA GRID BERITA - Disamakan persis dengan max-w-5xl & pembungkus minimalis */}
            ...
            {beritaData.map((berita) => (
              ...
              >
                ...
                <div className="mt-auto pt-4 text-sm font-bold text-[#0a1680] group-hover:text-[#f1b94c] flex items-center gap-1 transition-colors">
                  Detail <FiArrowRight size={14} className="group-hover:translate-x-1 transition-transform" />
                </div>
              </div>
            ))}
          </div>
