...
import { FiCalendar, FiArrowRight, FiFileText, FiClock, FiX, FiImage, FiRefreshCw } from "react-icons/fi"; 
import { News } from "@/types/news";
import { getNews } from "@/services/newsService";

export default function BeritaKeluargaPage() {
  ...
  useEffect(() => {
    async function fetchBerita() {
      try {
        ...
        // Filter berita yang dikhususkan untuk modul keluarga
        const beritaKeluarga = data.filter(
          (item) => item.modul === "keluarga" || item.category?.toLowerCase() === "keluarga"
        );
        
        setBeritaData(beritaKeluarga);
      } catch (error) {
        console.error("Gagal mengambil data berita keluarga:", error);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  return (
    <main className="bg-[#fcfdff] min-h-screen pb-24 font-sans pt-14 relative">
      ...
      {/* 1. HEADER (Identik dengan Kependudukan) */}
      <section className="pt-2 pb-20 px-6 text-center max-w-3xl mx-auto relative">
        ...
        <p className="text-gray-500 text-lg max-w-2xl mx-auto font-medium leading-relaxed">
          Kumpulan artikel, pengumuman, dan berita terbaru seputar program pemberdayaan Keluarga.
        </p>
      </section>
      ...
      <section className="max-w-5xl mx-auto px-6">
        {isLoading ? (
          ...
          <div className="text-center py-20 text-gray-500 flex flex-col items-center justify-center gap-2">
            <FiRefreshCw className="animate-spin text-[#0a1680]" size={32} />
            <p className="text-sm font-medium text-gray-400">Memuat berita keluarga...</p>
          </div>
        ) : beritaData.length === 0 ? (
          /* EMPTY STATE */
