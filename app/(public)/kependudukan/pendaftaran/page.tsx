...
export default function PendaftaranKependudukanPage() {
  const [formData, setFormData] = useState<RegistrationPayload>({
    program_id: "KEPENDUDUKAN-01", 
    full_name: "",
    nik: "",
    address: "",
    phone_number: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitStatus({ type: "success", message: "Pendaftaran berhasil! Data Anda telah masuk ke antrean layanan Kependudukan." });
      setFormData({ program_id: "KEPENDUDUKAN-01", full_name: "", nik: "", address: "", phone_number: "" });
    } catch {
      ...
      <section className="bg-[#0a1680] text-white pt-14 pb-28 overflow-hidden rounded-b-[4rem] shadow-lg relative">
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-6 text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold mb-4 tracking-tight">
            Registrasi <span className="text-[#f1b94c]">Kependudukan</span>
          </h1>
          <p className="text-[#93b2f8] font-medium text-lg leading-relaxed">
            Daftarkan diri Anda untuk layanan administrasi dan program kependudukan di Balai KB Payung Sekaki.
          </p>
        </div>
      </section>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="pt-4">
              <button type="submit" disabled={isLoading} className={`w-full flex items-center justify-center gap-2 py-4 rounded-xl font-bold text-white transition-all shadow-lg transform active:scale-95 ${isLoading ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#0a1680] hover:bg-[#f1b94c] hover:shadow-[#f1b94c]/30'}`}>
                <FiSend size={18} /> {isLoading ? "Mengirim Data..." : "Kirim Pendaftaran Kependudukan"}
              </button>
            </div>
          </form>
        </div>
